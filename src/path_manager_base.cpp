    ...
    nh_private_(ros::NodeHandle("~"))
{
    ...
    // PUBLISHER FOR DEBUGGING PURPOSES ONLY....

    _console_output_pub = nh_.advertise<std_msgs::String>("debug_messages", 1000); //-------------------------------------
...
void path_manager_base::waypoint_init() // NOT USED-----------------------------
{
...
{
  //std::cout << "received new waypoint from initializer\n";//------------------
  //ROS_DEBUG_STREAM("Hello " << "World");//------------------------------------
    ...
    dbg_msg << "Received new waypoint from path_planner (" << num_times_lt2 << ")";//-------------
    debug_message_publish(dbg_msg.str());//----------
}
...
void path_manager_base::debug_message_publish(std::string message) { //---
  std_msgs::String msg;
  std::stringstream ss;
  ss << message;
  msg.data = ss.str();
  _console_output_pub.publish(msg);
}
