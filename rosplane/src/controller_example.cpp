        ...
        else
        {
            switch (current_zone)
            {
                ...
                case alt_zones::CLIMB:
                    ...
                    output.theta_c = params.climb_angle_deg * 3.1415926539 /
                                     180.0; //airspeed_with_pitch_hold(input.Va_c, input.va, params, input.Ts);
                    if (input.h < input.h_c - params.alt_hz - 10.0)
                    {
                        output.delta_t = params.max_t;
                        output.theta_c = 22.5 * 3.1415926539 /
                                         180.0; //airspeed_with_pitch_hold(input.Va_c, input.va, params, input.Ts);
                    }
